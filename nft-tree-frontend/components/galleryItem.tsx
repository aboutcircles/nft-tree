...
export default function GalleryItem({ address, imageUrl, username, nftId, timestamp }: NFT) {
  return (
    <Link className="min-w-42 flex flex-col justify-between items-center border border-slate-300 text-xs hover:cursor-pointer portrait:lg:text-4xl" href={"https://circles.garden/profile/" + address} target="_blank">
      ...
      <div className="w-full flex flex-col p-2">
        <div className="w-full flex justify-between">
          <div className="flex flex-col">
            <p className="text-[10px] lg:text-[12px] portrait:lg:text-3xl text-white/50">MINTED BY</p>
            <p className="text-[8px] lg:text-[10px] portrait:lg:text-2xl">{username ? username : truncateAddress(address)}</p>
          </div>
        ...
        <p className="text-[10px] lg:text-[12px] portrait:lg:text-3xl text-white/50">MINTED ON</p>
        <p className="text-[8px] lg:text-[10px] portrait:lg:text-2xl">{new Date(Number(timestamp) * 1000).toDateString()}</p>
      </div>
    </Link>
  );
}
