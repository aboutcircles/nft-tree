...
contract CirclesTree is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function generateSVG(uint256 tokenId) public pure returns (string memory) {
        bytes memory svg = abi.encodePacked(
            ...
            '<text x="20%" y="7%" class="base" dominant-baseline="middle" text-anchor="middle">DEVCON 2024</text>'
            '<text x="84%" y="94%" class="base" dominant-baseline="middle" text-anchor="middle">',
            tokenId.toString(),
            "/1000 </text>"
    ...
    function getTokenURI(uint256 tokenId) public pure returns (string memory) {
        bytes memory dataURI = abi.encodePacked(
            "{",
            '"name": "Circles DevCon24 NFT#',
            tokenId.toString(),
            '",',
            '"description": "Minted at DevCon2024 on Gnosis Chain",',
            '"image": "',
            generateSVG(tokenId),
            '"',
            "}"
